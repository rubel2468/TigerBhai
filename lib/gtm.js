...
export function pushToDataLayer(eventName, payload = {}) {
  ...
  try {
    ...
    let aliasEvent = null
    if (lower === 'viewcontent') aliasEvent = 'view_content'
    if (lower === 'initiatecheckout') aliasEvent = 'initiate_checkout'
    // Keep add_to_cart as-is; GTM typically listens to add_to_cart already
    if (aliasEvent) {
      window.dataLayer.push({
        event: aliasEvent,
        ...payload,
